...
for s in sims:
    ...
    if count == 1:
        ...
        fh = tables.openFile("../s%d/s%d-mhw_phi_%d.h5" % (s,s,f))
    q = fh.root.StructGridField
    Xn, Yn, qn = projectOnFinerGrid_f39(Xc, Yc, q)

    pylab.subplot(1,2,count)
    pylab.title(title[count-1])
    pylab.pcolormesh(Xn, Yn, -pylab.transpose(qn))
    pylab.colorbar(orientation='horizontal')
    pylab.axis('image')
    count = count+1

pylab.savefig("mhw-cmp-phi_%d.png" % s, dpi=400)
pylab.close()
...
for s in sims:
    ...
    if count == 1:
        ...
        fh = tables.openFile("../s%d/s%d-mhw_chi_%d.h5" % (s,s,f))
    q = fh.root.StructGridField
    Xn, Yn, qn = projectOnFinerGrid_f39(Xc, Yc, q)
...
pylab.savefig("mhw-cmp-chi_%d.png" % s, dpi=400)
pylab.close()
...
for s in sims:
    ...
    f = 100
    if count == 1:
        fh = tables.openFile("../s%d/s%d-hw_numDens_%d.h5" % (s,s,f))
    else:
        fh = tables.openFile("../s%d/s%d-mhw_numDens_%d.h5" % (s,s,f))    
    q = fh.root.StructGridField
    Xn, Yn, qn = projectOnFinerGrid_f39(Xc, Yc, q)
...
pylab.savefig("mhw-cmp-numDens_%d.png" % s, dpi=400)
pylab.close()
