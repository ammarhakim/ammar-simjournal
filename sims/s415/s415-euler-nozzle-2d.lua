-- Script to do Euler equations with embedded boundary: flow in
-- convergent-divergent nozzle. See "Computational Fluid Dynamics",
-- J. D. Anderson, Jr.
...
uIn = 0.1 -- subsonic inflow
erIn = 0.5*rhoIn*uIn^2 + prIn/(gasGamma-1)

-- length of nozzle
XR = 3.0 -- [m]

-- nozzle shape function gives y as function of x
function nozzleY(x)
   local Ymax = math.sqrt((1 + 2.2*(0-1.5)^2)/Lucee.Pi)
   if (x<0) then
      return Ymax
   end
   local area = 1 + 2.2*(x-1.5)^2
   return math.sqrt(area/Lucee.Pi)
end
YM = math.max(nozzleY(0.0), nozzleY(XR))

-- function nozzleY(x)
--    local Ymax = 1 + 2.2*(0-1.5)^2
--    if (x<0) then
--       return Ymax
--    end
--    local area = 1 + 2.2*(x-1.5)^2
...
inOut:set(
   function (x,y,z)
      local yn = nozzleY(x)
      return math.abs(y)<yn and 1.0 or -1.0
   end
)
inOut:sync()
-- write field
...
-- initial conditions
function init(x,y,z)
   local rho, u, pr = 1.0, 0.1, 1.0
   if (x>0) then
      rho = 1-0.3146*x
      local T = 1-0.2314*x
      pr = rho*T
      u = (0.1+1.09*x)*math.sqrt(T)
   end
...
-- apply wall BCs on embedded boundaries
