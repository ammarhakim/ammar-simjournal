...
dx = 1/nx
X_ax = linspace(0+0.5*dx, 1.0-0.5*dx, nx)
...
fh = tables.openFile("s425-super-euler-3d_q_%d.h5" % 10)
...
for i in range(tRange.shape[0]):
    v = calcLineOut(nv, dx, dy, rho[:,:,iz], tRange[i])
    inOut_lo = calcLineOut(nv, dx, dy, inOut[:,:,iz], tRange[i])
    v_ma = numpy.ma.masked_where(inOut_lo<0, v)
    plot(Xr, v_ma, '-b')
    plot(X_ax, rho_ax[:,0,iz_ax], '-k', linewidth=2)

xlabel('Radial Location')
ylabel('Density')

savefig('s425-lineout-cmp.png', dpi=300, bbox_inches='tight')
show()
