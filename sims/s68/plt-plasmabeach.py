...
NT = 100
Ey_tx = numpy.zeros( (NT+1, 200), numpy.float )
for i in range(NT+1):
    fh = tables.openFile("s68-plasmabeach_q_%d.h5" % i)
    q = fh.root.StructGridField
    Ey = q[:,11]
    Ey_tx[i,:] = Ey

dx = 1/200.0
X = pylab.linspace(0.5*dx, 1-0.5*dx, 200)
...
pylab.savefig('s68-plasmabeach_Ey.png')
pylab.show()
