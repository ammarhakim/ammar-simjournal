...
-- maximum time-step (this is to avoid thrashing around)
dtMax = 0.001
...
incrDistf = DataStruct.Field2D {
   onGrid = grid,
   numComponents = basis:numNodes(),
   ghost = {1, 1},
}
totalDistf = DataStruct.Field2D {
   onGrid = grid,
   numComponents = basis:numNodes(),
   ghost = {1, 1},
}
...
hamilCalc = Updater.NonLinEmGke1DHamil {
   onGrid = grid,
   basis = basis,
...
function calcHamiltonian(curr, dt, phi1dIn, Apar1dIn, hamilOut)
   ...
   hamilOut:clear(0.0)
   -- compute Hamiltonian
   runUpdater(hamilCalc, curr, dt, {phi2d, Apar2d}, {hamilOut})
end
...
function updateVlasovEqn(curr, dt, Ht, distIn, distOut)
   -- calculate total-F
   totalDistf:combine(1.0, distfEquil, 1.0, distIn)

   -- compute increment from {f1+f0, H} term
   local s1, dt1 = runUpdater(pbSlvr, curr, dt, {totalDistf, Ht}, {incrDistf})
   ...
   -- combine terms
   distOut:combine(1.0, distIn, dt, incrDistf)
   return true, dt1
end
...
function rk3(tCurr, myDt)
   ...
   -- RK stage 1
   status, dtSuggested = updateVlasovEqn(tCurr, myDt, hamil, distf, distf1)
   ...
   -- RK stage 2
   status, dtSuggested = updateVlasovEqn(tCurr, myDt, hamil, distf1, distfNew)
   if (status == false) then
      return status, dtSuggested
   end
