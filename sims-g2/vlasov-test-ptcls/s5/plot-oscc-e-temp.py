...
def plotFig(i, c, lbl):
    ...
    data = pg.GData("s5-oscc-E_ions_M0_%d.bp" % i)
    dg = pg.data.GInterpModal(data, 2, "ms")
    XX, q0 = dg.interpolate()

    data = pg.GData("s5-oscc-E_ions_M1i_%d.bp" % i)
    dg = pg.data.GInterpModal(data, 2, "ms")
    ...
    data = pg.GData("s5-oscc-E_ions_M2_%d.bp" % i)
    dg = pg.data.GInterpModal(data, 2, "ms")
...
savefig('s5-temp-cmp-1d.png', dpi=150)
