...
function maxwellian(n0, vdrift, vt, v)
   local ux, uy, uz = vdrift[1], vdrift[2], vdrift[3]
   local vx, vy, vz = v[1], v[2], v[3]
   return n0/(2*math.pi*vt^2)^1.5*math.exp(-((vx-ux)^2+(vy-uy)^2+(vz-uz)^2)/(2*vt^2))
end
...
sim = Plasma.App {
   ...
   neut1 = Plasma.Species {
      charge = 0.0, mass = 1.0,
      -- velocity space grid
      lower = {-6.0*vThermal, -6.0*vThermal, -6.0*vThermal},
      upper = {6.0*vThermal, 6.0*vThermal, 6.0*vThermal},
      cells = {8, 8, 8},
	 ...
	 return maxwellian(n, {u, 0.0, 0.0}, vt, {vx, vy, vz})
      end,
