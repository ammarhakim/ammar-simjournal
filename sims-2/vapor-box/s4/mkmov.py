...
fh = tables.openFile("s4-four-box-chain_inOut.h5")
...
for i in range(10,11):
    ...
    fh = tables.openFile("s4-four-box-chain_q_%d.h5" % i)
    q = fh.root.StructGridField
    X, Y = getMeshGrid(fh.root.StructGrid)

    numDensity = q[:,:,0]/mLi
    mkFig(fh, X, Y, numDensity, 's4-four-box-chain_numDensity_%05d.png' % i, "Number Density")
    press = pressure(q)
    mkFig(fh, X, Y, press, 's4-four-box-chain_press_%05d.png' % i, "Pressure [Pa]")
    temp = press/(numDensity*kb)
    mkFig(fh, X, Y, temp-273.15, 's4-four-box-chain_temp_%05d.png' % i, "Temperature [C]")
    machN = mach(q)
    mkFig(fh, X, Y, machN, 's4-four-box-chain_mach_%05d.png' % i, "Mach Number")
