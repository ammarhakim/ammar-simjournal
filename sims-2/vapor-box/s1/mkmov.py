...
for i in range(11):
    ...
    numDensity = q[:,:,0]/mLi
    mkFig(fh, X, Y, numDensity, 's1-vapor-box-euler-kh_numDensity_%05d.png' % i, "Number Density")
    temp = pressure(q)/(numDensity*kb)
    mkFig(fh, X, Y, temp-273.15, 's1-vapor-box-euler-kh_temp_%05d.png' % i, "Temperature [C]")
    machN = mach(q)
    mkFig(fh, X, Y, machN, 's1-vapor-box-euler-kh_mach_%05d.png' % i, "Mach Number")
