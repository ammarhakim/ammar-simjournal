...
-- static dipole (electro)magnetic field
staticEB = DataStruct.Field3D {
   onGrid = grid,
   numComponents = 6,
   ghost = {2, 2},
}
...
function init(x,y,z)
   ...
   Bz = 0.0 -- zap out field as a test
...
elcFluidSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
...
ionFluidSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
...
maxSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
elcFluidSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = elcEulerEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
ionFluidSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = ionEulerEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
maxSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
elcFluidSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = elcEulerEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
ionFluidSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = ionEulerEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
maxSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "monotonized-centered",
   cfl = cfl,
...
elcLaxSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
...
ionLaxSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
...
maxLaxSlvrDir0 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "zero",
   cfl = cfl,
...
elcLaxSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = elcEulerLaxEqn,
   limiter = "zero",
   cfl = cfl,
...
ionLaxSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = ionEulerLaxEqn,
   limiter = "zero",
   cfl = cfl,
...
maxLaxSlvrDir1 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "zero",
   cfl = cfl,
...
-- ds solvers for Lax Euler equations along Y
elcLaxSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = elcEulerLaxEqn,
   limiter = "zero",
   cfl = cfl,
...
ionLaxSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = ionEulerLaxEqn,
   limiter = "zero",
   cfl = cfl,
...
maxLaxSlvrDir2 = Updater.WavePropagation3D {
   onGrid = grid,
   equation = maxwellEqn,
   limiter = "zero",
   cfl = cfl,
...
sourceSlvr = Updater.ImplicitFiveMomentSrc3D {
   onGrid = grid,
   numFluids = 2,
...
function updateFluidsAndField(tCurr, t)
   ...
   if ((elcEulerEqn:checkInvariantDomain(elcFluid) == false)
   ...
   -- Z-direction updates
   for i,slvr in ipairs({elcFluidSlvrDir2, ionFluidSlvrDir2, maxSlvrDir2}) do
      slvr:setCurrTime(tCurr)
      local status, dtSuggested = slvr:advance(t)
      myStatus = status and myStatus
      myDtSuggested = math.min(myDtSuggested, dtSuggested)
...
function updateFluidsAndFieldLax(tCurr, t)
   ...
   -- Z-direction updates
   for i,slvr in ipairs({elcLaxSlvrDir2, ionLaxSlvrDir2, maxLaxSlvrDir2}) do
      slvr:setCurrTime(tCurr)
      local status, dtSuggested = slvr:advance(t)
      myStatus = status and myStatus
      myDtSuggested = math.min(myDtSuggested, dtSuggested)
...
elcFluidSlvrDir1:setOut( {elcFluid} )
ionFluidSlvrDir1:setIn( {ionFluidX} )
ionFluidSlvrDir1:setOut( {ionFluid} )
maxSlvrDir1:setIn( {emFieldX} )
maxSlvrDir1:setOut( {emField} )

elcFluidSlvrDir2:setIn( {elcFluid} )
elcFluidSlvrDir2:setOut( {elcFluidNew} )
ionFluidSlvrDir2:setIn( {ionFluid} )
ionFluidSlvrDir2:setOut( {ionFluidNew} )
maxSlvrDir2:setIn( {emField} )
maxSlvrDir2:setOut( {emFieldNew} )
...
elcLaxSlvrDir1:setOut( {elcFluid} )
ionLaxSlvrDir1:setIn( {ionFluidX} )
ionLaxSlvrDir1:setOut( {ionFluid} )
maxLaxSlvrDir1:setIn( {emFieldX} )
maxLaxSlvrDir1:setOut( {emField} )

elcLaxSlvrDir2:setIn( {elcFluid} )
elcLaxSlvrDir2:setOut( {elcFluidNew} )
ionLaxSlvrDir2:setIn( {ionFluid} )
ionLaxSlvrDir2:setOut( {ionFluidNew} )
maxLaxSlvrDir2:setIn( {emField} )
maxLaxSlvrDir2:setOut( {emFieldNew} )
