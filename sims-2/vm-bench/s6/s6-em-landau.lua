...
phaseGridElc = Grid.RectCart3D {
   lower = {-LX/2, VL_ELC, VL_ELC},
   upper = {LX/2, VU_ELC, VU_ELC},
   cells = {NX, NVX, NVY},
   periodicDirs = {0},
   decomposition = phaseDecomp,   
}
...
phaseGridIon = Grid.RectCart3D {
   lower = {-LX/2, VL_ION, VL_ION},
   upper = {LX/2, VU_ION, VU_ION},
   cells = {NX, NVX, NVY},
   periodicDirs = {0},   
   decomposition = phaseDecomp,   
}
...
initDistfElc = Updater.ProjectOnNodalBasis3D {
   ...
   evaluate = function(x,vx,vy,t)
      ...
      return maxwellian(nElc, elcDrift, elcDrift, vtElc, vx, vy)
   end
}
...
initDistfIon = Updater.ProjectOnNodalBasis3D {
   ...
   -- function to use for initialization
   evaluate = function(x,vx,vy,t)
      return maxwellian(n0, ionDrift, ionDrift, vtIon, vx, vy)
   end
}
