...
-- for antenna current
antennaCurrent = DataStruct.Field1D {
   onGrid = confGrid,
   numComponents = 2*confBasis:numNodes(),
   ghost = {1, 1},
}
...
-- updater to compute currents from antenna: for production RF
-- problems, this should be coded in C++.
antennaCurrentSrc = Updater.ProjectOnNodalBasis1D {
   onGrid = confGrid,
   basis = confBasis,
   shareCommonNodes = false, -- In DG, common nodes are not shared
   -- function to use for initialization
...
function rkStage(tCurr, dt, elcIn, ionIn, emIn, elcOut, ionOut, emOut)
   ...
   -- compute antenna currents
   runUpdater(antennaCurrentSrc, tCurr, dt, {}, {antennaCurrent})
   current:combine(elcCharge, momentumElc, ionCharge, momentumIon, 1.0, antennaCurrent)
