...
Tr_Tl = 4.0 -- ratio of right/left beam temperatures
...
VL_ELC, VU_ELC = -6.0*math.sqrt(Tr_Tl)*vtElc, 6.0*math.sqrt(Tr_Tl)*vtElc
VL_ION, VU_ION = -10.0*math.sqrt(Tr_Tl)*vtIon-ionDrift, 10.0*math.sqrt(Tr_Tl)*vtIon+ionDrift
...
initDistfElc = Updater.ProjectOnNodalBasis2D {
   ...
   evaluate = function(x,v,z,t)
      ...
      if x>sloc then
	 fxv = maxwellian(n0, -math.sqrt(Tr_Tl)*elcDrift, math.sqrt(Tr_Tl)*vtElc, v)
      end
      return fxv
   end
}
...
initDistfIon = Updater.ProjectOnNodalBasis2D {
   ...
   evaluate = function(x,v,z,t)
      ...
      if x>sloc then
	 fxv = maxwellian(n0, -math.sqrt(Tr_Tl)*ionDrift, math.sqrt(Tr_Tl)*vtIon, v)
      end
      return fxv
   end
}
