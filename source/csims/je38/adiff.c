...
{
  return -(frr-16*fr+fll-16*fl+30*f0)/(12*dx*dx);
}
...
adiff_app_new(const struct adiff_app_inp *inp)
{
  ...
  app->a_scheme = inp->a_scheme;
  app->d_scheme = inp->d_scheme;
...
calc_velocity_field(adiff_app *app, double tm)
{
  struct gkyl_offset_descr offsets[] = {
    ...
    { 0.0, 0.0, -0.5 }, // uz is at back face
  };
  ...
  struct gkyl_array *rhs)
{
  ...
  // labels for five cells: two to the left, itself, two to the right
