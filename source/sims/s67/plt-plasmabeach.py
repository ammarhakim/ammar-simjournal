...
NT = 100
Ey_tx = numpy.zeros( (NT+1, 100), numpy.float )
for i in range(NT+1):
    fh = tables.openFile("s67-plasmabeach_q_%d.h5" % i)
    q = fh.root.StructGridField
    Ey = q[:,11]
    Ey_tx[i,:] = Ey

dx = 1/100.0
X = pylab.linspace(0.5*dx, 1-0.5*dx, 100)
...
pylab.savefig('s67-plasmabeach_Ey.png')
pylab.show()
