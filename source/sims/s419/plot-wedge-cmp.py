...
fh = tables.openFile("s419-euler-wedge-2d_q_10.h5")
...
fh = tables.openFile("s419-euler-wedge-2d_inOut.h5")
...
contour(X, Y, transpose(rhoMa), colors='k')
im = pcolormesh(X, Y, transpose(rhoMa))
ylabel('Y')
gca().set_xticklabels([])
title('Density')
axis('image')
...
subplot(2,2,2)
contour(X, Y, transpose(prMa), colors='k')
...
###
fh = tables.openFile("../s418/s418-euler-wedge-2d_q_10.h5")
q = fh.root.StructGridField
gasGamma = 1.4
...
fh = tables.openFile("../s418/s418-euler-wedge-2d_inOut.h5")
inOut = fh.root.StructGridField.read()
...
savefig('s418-419-euler-wedgecmp.png', dpi=300)
