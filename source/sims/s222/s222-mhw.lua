...
-- create FEM nodal basis on X grid
basis1d = NodalFiniteElement1D.Lobatto {
   -- grid on which elements should be constructured
   onGrid = grid1d,
   -- polynomial order in each cell. One of 1, or 2. Corresponding
   -- number of nodes are 4 and 8.
...
-- non-zonal component of number density
numDensNZ = DataStruct.Field2D {
   onGrid = grid,
   numComponents = 1*numDgNodesPerCell,
   ghost = {1, 1},
}
...
-- 1D fields to store zonal component of a field
zonal1d = DataStruct.Field1D {
   onGrid = grid1d,
   numComponents = basis1d:numNodes(),
   ghost = {1, 1},
}
...
--  non-zonal component of potential
phiNZ = DataStruct.Field2D {
   onGrid = grid,
   -- numNodesPerCell is number of global nodes stored in each cell
   numComponents = 1*numDgNodesPerCell,
   ghost = {1, 1},
...
-- to compute moment
momentCalc = Updater.DistFuncMomentCalc1D {
   -- 2D grid 
   onGrid = grid,
   -- 2D basis functions
   basis2d = basis,
...
-- compute average of a 2D field and store in 1D field
function calcMoment(curr, dt, in2D, out1D)
   momentCalc:setCurrTime(curr)
   momentCalc:setIn( {in2D} )
   momentCalc:setOut( {out1D} )
   momentCalc:advance(curr+dt)
   -- apply BC to output field
   out1D:applyPeriodicBc(0)
end

-- compute nonzonal component of a field
function calcNonZonal(curr, dt, inpTotal, outNonZonal)
   -- compute moment
   calcMoment(curr, dt, inpTotal, zonal1d)
   -- scale by domain size
   zonal1d:scale(-1/LY)
   -- copy it over to output fiel
   copyTo2D:setCurrTime(curr)
   copyTo2D:setIn( {zonal1d} )
   copyTo2D:setOut( {outNonZonal} )
   copyTo2D:advance(curr+dt)
   -- apply BC to output field
   applyBc(outNonZonal)
   -- accumulate original field to compute nonzonal component
   outNonZonal:accumulate(1.0, inpTotal)
end
...
function rk3(tCurr, myDt)
   ...
   calcNonZonal(tCurr, myDt, phiDG, phiNZ)
   calcNonZonal(tCurr, myDt, numDens, numDensNZ)
   -- accumulate source term into updated solutions
   numDens1:accumulate(-coupleCoeff*myDt, phiNZ, -coupleCoeff*myDt, numDensNZ)
   chi1:accumulate(-coupleCoeff*myDt, phiNZ, -coupleCoeff*myDt, numDensNZ)
   ...
   -- compute non-zonal components
   calcNonZonal(tCurr, myDt, phiDG, phiNZ)
   calcNonZonal(tCurr, myDt, numDens1, numDensNZ)
   -- accumulate source term into updated solutions
   numDensNew:accumulate(-coupleCoeff*myDt, phiNZ, -coupleCoeff*myDt, numDensNZ)
   chiNew:accumulate(-coupleCoeff*myDt, phiNZ, -coupleCoeff*myDt, numDensNZ)
