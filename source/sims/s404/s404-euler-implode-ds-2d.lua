...
-- this global table stores if we are taking an odd or even step
globalStateSwitch = { isEven = true }
-- switch state
function switchState()
   if (globalStateSwitch.isEven) then
      globalStateSwitch.isEven = false
   else
      globalStateSwitch.isEven = true
   end
end
...
function updateFluidsAndField(tCurr, t)
   ...
   local updatersDir0, updatersDir1
   if (globalStateSwitch.isEven) then
      updatersDir0 = {fluidSlvrDir0}
      updatersDir1 = {fluidSlvrDir1}
   else
      updatersDir0 = {fluidSlvrDir1}
      updatersDir1 = {fluidSlvrDir0}
   end

   -- X-direction updates
   for i,slvr in ipairs(updatersDir0) do
      slvr:setIn( {fluid} )
      slvr:setOut( {fluidX} )
   ...
   applyBc(fluidX, tCurr, t-tCurr)

   -- Y-direction updates
   for i,slvr in ipairs(updatersDir1) do
      slvr:setIn( {fluidX} )
...
function runSimulation(tStart, tEnd, nFrames, initDt)
   ...
   while true do
      ...
      elseif (useLaxSolver == true) then
        ...
        myDt = dtSuggested
        step = step + 1
	-- switch state before next time-step
	--switchState()
