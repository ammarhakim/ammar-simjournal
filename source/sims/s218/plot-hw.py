...
fh = tables.openFile("s218-hw_chi_0.h5")
...
for i in range(nFrame):
    fh = tables.openFile("s218-hw_chi_%d.h5" % i)
...
for i in range(nFrame):
    ...
    pylab.savefig('s218-hw_chi_%05d.png' % i)
    pylab.close()
...
vmin = []
for i in range(nFrame):
    fh = tables.openFile("s218-hw_numDens_%d.h5" % i)
    q = fh.root.StructGridField.read()
    vmin.append(q.min())
    vmax.append(q.max())
    fh.close()
...
for i in range(nFrame):
    ...
    pylab.savefig('s218-hw_numDens_%05d.png' % i)
    pylab.close()
...
vmin = []
for i in range(nFrame):
    fh = tables.openFile("s218-hw_phi_%d.h5" % i)
    q = fh.root.StructGridField.read()
    vmin.append(q.min())
    vmax.append(q.max())
    fh.close()
...
for i in range(nFrame):
    ...
    pylab.savefig('s218-hw_phi_%05d.png' % i)
    pylab.close()
