...
ionPlasmaFreq = math.sqrt(n0*ionCharge^2/(epsilon0*ionMass))
...
function initElc(x,y,z)
   ...
   local ezmom = -(1.0/6.0)*B0*(1/lambda)*(1/math.cosh((y-Ly4)/lambda)^2 - 1/math.cosh((y-3*Ly4)/lambda)^2)*(me/qe)
...
function initIon(x,y,z)
   ...
   -- ion momentum is computed from plasma current that supports field
   local izmom = -(5.0/6.0)*B0*(1/lambda)*(1/math.cosh((y-Ly4)/lambda)^2 - 1/math.cosh((y-3*Ly4)/lambda)^2)*(mi/qi)
...
function initEmField(x,y,z)
   ...
   local dBx1 = psi0*(4*pi/Ly)*sin(2*pi*x/Lx)*sin(4*pi*y/Ly)
   local dBy1 = -psi0*(2*pi/Lx)*cos(2*pi*x/Lx)*(1 - cos(4*pi*y/Ly))
