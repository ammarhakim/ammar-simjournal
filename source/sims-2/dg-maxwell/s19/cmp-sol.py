...
def projectOnFinerGrid_f2(Xc, Yc, q):
    dx = Xc[1]-Xc[0]
    dy = Yc[1]-Yc[0]
    nx = Xc.shape[0]
    ny = Yc.shape[0]
...
fh = tables.openFile("s19-dg-maxwell_q_1.h5")
...
Xn, Yn, qn_1 = projectOnFinerGrid_f3(Xc, Yc, q1)

pylab.figure(1)

pylab.subplot(1,2,2)
pylab.pcolormesh(Xn, Yn, pylab.transpose(qn_1))
pylab.title('p=2, 20x20 mesh')
pylab.axis('image')

## compare to
fh = tables.openFile("../s16/s16-dg-maxwell_q_1.h5")
grid = fh.root.StructGrid
lower = grid._v_attrs.vsLowerBounds
upper = grid._v_attrs.vsUpperBounds
cells = grid._v_attrs.vsNumCells
...
Xn, Yn, qn_1 = projectOnFinerGrid_f2(Xc, Yc, q1)

pylab.subplot(1,2,1)
pylab.pcolormesh(Xn, Yn, pylab.transpose(qn_1))
pylab.title('p=1, 40x40 mesh')
pylab.axis('image')

pylab.savefig('s16-s19-dg-maxwell-Ez.png', bbox_inches='tight')
