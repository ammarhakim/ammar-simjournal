...
cmin = min(chiS.min(), chiC.min())
cmax = max(chiS.max(), chiC.max())

figure(1)
suptitle('Vorticity: Global (left), Coupled (right). T=5')
fig = subplot(1,2,1)
im = pcolormesh(X[0], X[1], transpose(chiS), cmap='inferno', vmin=cmin, vmax=cmax)
set
axis('image')
colorbar_adj(im)

subplot(1,2,2)
im = pcolormesh(X[0], X[1], transpose(chiC), cmap='inferno', vmin=cmin, vmax=cmax)
plot([0, 0], gca().get_ylim(), 'w--')
plot([5, 5], gca().get_ylim(), 'w--')
axis('image')
colorbar_adj(im)

savefig('c3-coupled-chi-cmp-t5.png', dpi=300)

X1 = linspace(-20, 20, 96)
f = figure(3)
...
# Late time
d = pg.GData("c3-hw_chiFull_10.h5")
dgC = pg.data.interp.GInterpNodalSerendipity(d, 2)
X, chiC = dgC.project(0)
...
savefig('c3-coupled-chi-cmp-t10.png', dpi=300)
