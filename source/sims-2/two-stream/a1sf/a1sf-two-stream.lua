...
-- positivity rescaled distribution function for electrons
distfPosScaled = DataStruct.Field2D {
   onGrid = phaseGrid,
   numComponents = 1*phaseBasis:numNodes(),
   ghost = {1, 1},
}
...
pbSlvr = Updater.PoissonBracket {
   ...
   applyPositivityFix = false, -- apply positivity fix
}
...
function poissonBracket(curr, dt, distfIn, hamilIn, distfOut)
   return runUpdater(pbSlvr, curr, dt, {distfIn, hamilIn, distfPosScaled}, {distfOut})
end
...
function rk3(tCurr, myDt)
   ...
   applyPhaseBc(distf1)
   distfPosScaled:copy(distf1)
   runUpdater(scalingPositivityLimiter, tCurr, myDt, {}, {distfPosScaled})
