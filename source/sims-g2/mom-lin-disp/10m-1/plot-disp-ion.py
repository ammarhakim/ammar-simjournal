...
vthi = math.sqrt(temperature/mass)
rhoi = vthi/omegac
...
figure(1)
for i in range(16):
    plot(rhoi*k_5m, abs(omega_r_5m[:,i]), '.', color='r', markersize=2)

for i in range(14):
    plot(rhoi*k_10m, abs(omega_r_10m[:,i]), '.', color='k', markersize=2)

grid()
xlabel(r"$k \rho_{i}$")
ylabel(r"$\omega_r$")

gca().set_xlim([rhoi*0.0, rhoi*3.0])
gca().set_ylim([0.0, 0.2])

text(rhoi*0.01, 0.05 + 0.01, "$\omega_{ci}$")
text(rhoi*0.01, 0.1 + 0.01, "$2 \omega_{ci}$")
