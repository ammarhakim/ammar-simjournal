...
for i in range(86):
    ...
    # ions
    data = pg.GData("n5-Lz4-no-collisions_ion_GkM0_%d.bp" % i)
    ...
    # elc
    data = pg.GData("n5-Lz4-no-collisions_electron_GkM0_%d.bp" % i)
    dg = pg.data.GInterpModal(data, 1, "ms")
    XX, q0 = dg.interpolate()
    X, Y = meshgrid(XX[0], XX[1])
    nz2 = int(q0.shape[2]/2)
    ...
    savefig("n5-Lz4-no-collisions_GkM0_%05d.png" % i, dpi=200)
    close()
